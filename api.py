...
class PubMed(object):
    ...
    def query(self: object, query: str, max_results: int = -1):
    ...
    def getTotalResultsCount(self: object, query: str) -> int:
        ...
        total_results_count = int(response.get(
    ...
    def _exceededRateLimit(self) -> bool:
        ...
        self._requestsMade = [requestTime for requestTime in self._requestsMade if requestTime >
    ...
    def _getArticleIds(self: object, query: str, max_results: int) -> list:
        ...
        # Add specific query parameters
        parameters["term"] = query
        parameters["retmax"] = 9999

        if max_results < parameters["retmax"] and max_results != -1:
            ...
            for i in range(1946, 2021):
                date = f'{i}/01/01:{i}/06/01 [dp]'
                parameters["term"] = query + date
                response = self._get(
                    url="/entrez/eutils/esearch.fcgi", parameters=parameters)
                article_ids += response.get("esearchresult",
                                            {}).get("idlist", [])

                date = f'{i}/06/02:{i}/12/31 [dp]'
                parameters["term"] = query + date
                response = self._get(
                    url="/entrez/eutils/esearch.fcgi", parameters=parameters)
                article_ids += response.get("esearchresult",
            ...
            # total_result_count = self.getTotalResultsCount(query)
            # retrieved_count = int(response.get(
            #     "esearchresult", {}).get("retmax"))
            # retrieved_count = parameters["retmax"]
            # print(total_result_count, retrieved_count)

            # parameters["retstart"] = 9998
            # print('a')
            # response = self._get(
            #     url="/entrez/eutils/esearch.fcgi", parameters=parameters)
            # parameters["retstart"] = 9999
            # print('b')
            # response = self._get(
            #     url="/entrez/eutils/esearch.fcgi", parameters=parameters)

            # while retrieved_count < total_result_count:
            ...
            #                                 {}).get("idlist", [])
            #     print('c')
            #     # retrieved_count += int(response.get("esearchresult",
            #     #                        {}).get("retmax"))
            #     retrieved_count += parameters["retmax"]
            #     print('d')
        ...
        # total_result_count = int(response.get(
        #     "esearchresult", {}).get("count"))
